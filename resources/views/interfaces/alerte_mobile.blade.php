...
@section('name', 'ALERTE TRANSFEREE')
@section('body')
    ...
    <style>
        /* =============================================
           DESIGN PREMIUM - PAGE ALERTE TRANSFEREE
        ============================================= */
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h6 style="color:rgba(0, 0, 0, 0.6);">{{ strtoupper(Auth::user()->name) }}&nbsp; <i
                            class="zmdi zmdi-chevron-right"></i> &nbsp; Alerte transferée</h6>
                </div>
                                                ...
                                                <td style="text-align: center; padding-top: 5px; padding-bottom: 5px;">
                                                    <div class="dropdown-wrapper">
                                                        ...
                                                        <div id="menu-{{ $i }}" class="custom-dropdown-menu">
                                                            <!-- Option Désactiver / Déjà désactivé selon etat_2 -->
                                                            @if ($data->etat_2 == 1)
                                                                <button class="dropdown-item alert-action" data-id="{{ $data->id }}"
                                                                    data-nom="{{ User::where(['id' => $data->user_id])->first()['name'] ?? '' }}"
                                                                    data-modal-trigger="btn_ac" data-element-id="element_1">
                                                                    <i class="zmdi zmdi-settings text-danger"></i> Désactiver
                                                                </button>
                                                            @else
                                                                <button class="dropdown-item alert-action" data-id="{{ $data->id }}"
                                                                    data-nom="{{ User::where(['id' => $data->user_id])->first()['name'] ?? '' }}"
                                                                    data-modal-trigger="btn_cll" data-element-id="element_3">
    ...
    <span id="alerte_active" style="display: none;">{{ Alertes::where(['supprimer' => 0, 'etat_2' => 1])->where('user_id_transfert', '!=', 0)->get()->count(); }}</span>
    ...
    <script>
        $(document).ready(function() {
            ...
            // 1. Désactiver / Déjà désactivé (utilise etat_2)
            ...
            // Désactiver (oui_1) - utilise etat_2
            $("#oui_1").click(function(e) {
                e.preventDefault();
                var id = $("#data_id").html();
                $.get("{{ url('/refresh_desactiver_alerte_centrale_1') }}", {
                    id: id,
                }, function(refresh) {
            ...
            setInterval(function() {
                ...
                $.get("{{ url('/count_alerte_etat_2') }}", {}, function(nombre) {
                    if (nombre != alerte_active) {
                        jouerSonAlerte();
                        $("#alerte_active").html(nombre);
                        $.get("{{ url('/refresh_alerte_centrale_1') }}", {}, function(refresh) {
                            $("#content_groupe").html(refresh);
                            checkAndTriggerAlert();
                        });
                    } else {
