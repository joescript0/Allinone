...
<section class="content">
    ...
    <div style="margin-top: 30px;" class="container">
        <div class="row">
            <div class="col-lg-12">
                <h6 style="color:rgba(0, 0, 0, 0.6);">{{ strtoupper(Auth::user()->name) }}&nbsp; <i class="zmdi zmdi-chevron-right"></i> &nbsp; Types de dépenses</h6>
            </div>
                            ...
                            <table class="table table-bordered mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="padding-top: 5px;padding-bottom: 5px;">Code comptable</th>
                                        ...
                                        <td style="text-align: center;padding-top: 5px;padding-bottom: 5px;">
                                            ...
                                            <script>
                                                $("#edit_<?= $i ?>").click(function(e) {
                                                    e.preventDefault();
                                                    $.get("{{ url('/refresh_edit_type_frais') }}", {
                                                        type_frais_id: <?= $data->id ?>,
                                                    }, function(refresh_edit_type_frais) {
                                                        $("#bloc_1").hide();
                                                        $("#bloc_2").hide();
                                                        $("#bloc_3").show();
                                                        $("#bloc_3").html(refresh_edit_type_frais);
                                                    });
                                                });
            ...
            <div id="bloc_2" style="margin-top: 12px;display: none;" class="col-lg-12">
                <h4 style="color:rgba(0, 0, 0, 0.6);"><i style="font-size: 40px;" class="zmdi zmdi-money-box text-info"></i> Ajouter</h4>
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="code" name="code" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Code compatable (Ex : 001)">
                            </div>
                        </div>
                    </div>
...
<script>
    $("#link_14").css("border-left", "1px solid rgb(33, 150, 243)");
    $("#text_14").addClass("text-info");
    $("#icone_14").css("color", "rgb(33, 150, 243)");
    ...
    $("#save").click(function(e) {
        ...
        if (nom.trim().length == 0) {
            ...
            $.ajax({
                type: "POST",
                url: "/check_type_frais",
                data: data,
                success: function(response) {
                    if (response == 1) {
                        $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Ce type de frais existe déjà');
                        $('#msg').css('color', "#ff6b68");
                        setTimeout(() => {
                            $('#msg').html("");
                        }, 9000);
                        ...
                        if (code.trim().length == 0) {
                            $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Completez le code comptatble');
                            $('#msg').css('color', "#ff6b68");
                            setTimeout(() => {
                                $('#msg').html("");
                            }, 9000);
                            ...
                            if (!Number(code)) {
                                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Completez un bon code comptable');
                                $('#msg').css('color', "#ff6b68");
                                setTimeout(() => {
                                    $('#msg').html("");
                                }, 9000);
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        ...
                                        $("#code").val("");
                                        $("#description").val("");
                                        $('#msg').html('<i class="zmdi zmdi-check-circle"></i> Type de dépense ajouté avec succès');
    ...
    $("#oui").click(function(e) {
        e.preventDefault();
        var id = $("#data_id").html();
        $.get("{{ url('/refresh_delete_type_frais') }}", {
            id: id,
