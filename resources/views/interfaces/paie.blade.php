...
@section('body')
   ...
   ============================================================ */
        ...
        .table tbody td a i.zmdi-print {
            color: #4b5563;
        }
        .table tbody td a i.zmdi-delete {
            color: #ef4444;
        }
        .table tbody td a i.zmdi-lock {
            color: #6b7280;
        }
        ...
        .table tbody td a:hover i.zmdi-print {
            color: #1f2937;
        }
        ...
        /* ========== MODALE DE PAIEMENT ========== */
        .modal.fade#modal-centered .modal-dialog {
            max-width: 500px;
        }
        .modal.fade#modal-centered .modal-content {
            border-radius: var(--border-radius-lg);
            border: none;
            box-shadow: var(--shadow-premium);
            overflow: hidden;
        }
        .modal.fade#modal-centered .modal-header {
            background: var(--bleu-nuit-gradient) !important;
            border-bottom: none;
            padding: 1rem 1.5rem;
        }
        .modal.fade#modal-centered .modal-header .modal-title {
            font-weight: 700;
            color: white;
        }
        .modal.fade#modal-centered .modal-header .close {
            color: white;
            opacity: 0.8;
        }
        .modal.fade#modal-centered .modal-header .close:hover {
            opacity: 1;
        }
        .modal.fade#modal-centered .modal-footer {
            background: #f8fafc;
            border-top: 1px solid #eef2f6;
            padding: 1rem 1.5rem;
        }
        .modal.fade#modal-centered .modal-footer .btn {
            border-radius: 40px !important;
            padding: 6px 18px !important;
            font-weight: 600;
        }
        .modal.fade#modal-centered .table thead th {
        ...
        .modal.fade#modal-centered .table tbody td {
            padding: 6px 8px !important;
            font-size: 0.85rem;
        }
        #m_paie {
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h6 style="color:rgba(0, 0, 0, 0.6);">{{ strtoupper(Auth::user()->name) }}&nbsp; <i
                            class="zmdi zmdi-chevron-right"></i> &nbsp; Liste de paie</h6>
                </div>
                                        ...
                                        @foreach ($listespaies as $data)
                                            <tr>
                                                ...
                                                <td style="padding-top: 5px;padding-bottom: 5px;">
                                                    ...
                                                    @endif
                                                    @if ($data->etat == 1)
                                                        <i class="zmdi zmdi-block text-info"></i> <span
                                                            class="text-info">Activé </span>
                                                    @endif
                                                    @if ($data->etat == 2)
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div style="border: 1px solid black;" class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" id="save_paie" class="btn btn-info">Enregistrer <i
                            class="zmdi zmdi-save"></i></button>
                    <button type="button" id="annuler_paie" class="btn btn-danger" data-dismiss="modal">Close <i
        ...
        $("#save").click(function(e) {
            ...
            if (annee_id.trim().length == 0) {
                ...
                if (moi_id.trim().length == 0) {
                    ...
                    $.get("{{ url('/check_solde_1') }}", {
                        annee_id: annee_id,
                        moi_id: moi_id
                    }, function(rep) {
                        if (rep != 0) {
                            $('#msg').html(
                                '<i class="zmdi zmdi-close-circle"></i> Cette liste de paie existe');
                            $('#msg').css('color', "#ff6b68");
                            setTimeout(() => {
                                $('#msg').html("");
                            }, 9000);
                        } else {
                            $.get("{{ url('/check_solde_encours_1') }}", {}, function(resp_solde) {
                                var rr = resp_solde.split("__________");
                                if (rr[0] != 0) {
                                    $('#msg').html(
                                        '<i class="zmdi zmdi-close-circle"></i> La liste de ' +
                                        rr[1] + ' est activé.');
                                    $('#msg').css('color', "#ff6b68");
                                    setTimeout(() => {
                                        $('#msg').html("");
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            ...
                                            $('#msg').html(
                                                '<i class="zmdi zmdi-check-circle"></i> liste de paie ajouté avec succès'
                                            );
