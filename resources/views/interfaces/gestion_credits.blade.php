...
<style>
   ...
   ============================================================ */
    ...
    /* ========== BOUTONS D'ACTION DANS LE TABLEAU (Rembourser / Créditer) ========== */
    .table tbody td a {
    ...
    .table tbody td a.text-success:hover {
        background: #cce8df;
    ...
    /* ========== TOTAUX (Crédit total, Remboursement total, Reste) ========== */
    .credit-totals {
        display: flex;
    ...
    .credit-totals .text-info { color: #2c7da0; }
    .credit-totals .text-success { color: #0a5c3e; }
    .credit-totals .text-danger { color: #b91c1c; }
    ...
    /* ========== FORMULAIRE CRÉDIT : CHAMP MONTANT + DEVISE ========== */
                            ...
                            <div class="col-12" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center;">
                                ...
                                <?php if ((Writes::where(["ressource_id" => $ressource_id_1, "groupe_id" => $groupe_user_id])->get()->count() != 0) || (Auth::user()->role == 0)) { ?>
                                    ...
                                    <?php } else { ?>
                                        ...
                                        &nbsp;
                                        <a id="print_r" class="btn-secondary btn-sm" style="background: #cbd5e1 !important; color: #475569 !important; cursor: not-allowed; opacity: 0.7;">
                                            <i class="zmdi zmdi-print"></i> Imprimer
                                        </a>
                                    <?php } ?>
                                <?php } ?>
        ...
        <div class="row">
            ...
            <div id="bloc_1" style="margin-top: 12px;" class="col-lg-12">
                <h4 style="color:rgba(0, 0, 0, 0.6);">
                    ...
                    Liste
                    <span class="credit-count-badge">
                        <i class="zmdi zmdi-view-list"></i> <span id="creditCount">0</span>
                    </span>
                </h4>
                ...
                <div class="filters-container">
                    <div class="filter-group">
                        <label><i class="zmdi zmdi-calendar text-danger"></i> Date crédit</label>
                    ...
                    <div class="filter-group">
                        <label><i class="zmdi zmdi-label text-danger"></i> Type</label>
                        <input type="text" id="filterType" class="form-control" placeholder="E ou C...">
                    </div>
                    <div class="filter-group">
                        <label><i class="zmdi zmdi-money text-danger"></i> Montant</label>
                        <input type="number" id="filterMontant" class="form-control" placeholder="Montant exact" step="0.01" min="0">
                    </div>
                ...
                <div class="credit-totals">
                    <span class="text-info"><i class="zmdi zmdi-money"></i> Crédit total : <span id="totalCredit">0,00</span> $</span>
                    <span class="text-success"><i class="zmdi zmdi-check-circle"></i> Remboursement total : <span id="totalRemboursement">0,00</span> $</span>
                    <span class="text-danger"><i class="zmdi zmdi-close-circle"></i> Reste total : <span id="totalReste">0,00</span> $</span>
                </div>
                            ...
                            <table class="table table-bordered mb-0">
                                ...
                                <tbody id="creditsTableBody">
                                    ...
                                    <tr id="row_{{ $data->id }}">
                                        ...
                                        ?>
                                        <td class="date-cell" data-date="{{ $data->date_credit }}" style="padding-top: 5px;padding-bottom: 5px;">{{ $data->date_credit }}</td>
                                        <td class="client-cell" data-client="{{ $data->nom_credit }}" style="padding-top: 5px;padding-bottom: 5px;">{{ $data->nom_credit }}</td>
                                        <td class="type-cell" data-type="{{ $data->type }}" style="padding-top: 5px;padding-bottom: 5px;">{{ $data->type }}</td>
                                        <td class="montant-cell" data-montant="{{ $data->entree }}" data-rembourse="{{ $t }}" style="padding-top: 5px;padding-bottom: 5px;">
                                    ...
                                    <tr id="noResultRow" style="display: none;">
                                        <td colspan="5">
                                            <i class="zmdi zmdi-info-outline"></i> Aucun crédit ne correspond à vos critères.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td style="text-align: center;padding-top: 5px;padding-bottom: 5px;">
                                                ...
                                                <select class="form-control" id="devise_credit" name="devise_credit" style="width:85px; display:inline-block; vertical-align:middle; margin-left:4px; height:40px;">
                    ...
                    <div style="margin-top: 20px;" class="row">
                        <div class="col-12">
                            ...
                            <?php if ((($add == 1) && (Writes::where(["ressource_id" => $ressource_id_1, "groupe_id" => $groupe_user_id])->get()->count() != 0)) || (($add == 0) && (Auth::user()->role == 0))) { ?>
                                <button id="save" class="btn btn-info btn-sm">Enregistrer <i class="zmdi zmdi-save"></i></button>
                            <?php } else { ?>
                                <button id="save_r" class="btn btn-secondary btn-sm" style="background: #cbd5e1 !important; color: #475569 !important; cursor: not-allowed; opacity: 0.7;">Enregistrer <i class="zmdi zmdi-save"></i></button>
                            <?php } ?>
...
<script>
    $(document).ready(function() {
        ...
        // =================================================================
        const STORAGE_KEY = 'credits_filters';
        ...
        function loadFilters() {
            ...
            try {
                ...
                if (filters.client !== undefined) $('#filterClient').val(filters.client);
                if (filters.type !== undefined) $('#filterType').val(filters.type);
                if (filters.montant !== undefined) $('#filterMontant').val(filters.montant);
                return true;
            } catch (e) {
                return false;
            }
        ...
        // =================================================================
        // 2. FONCTION DE FILTRAGE + COMPTEUR + TOTAUX
        // =================================================================
        function filterCredits() {
            const filterDate = String($('#filterDate').val() || '').trim();
            const filterClient = String($('#filterClient').val() || '').toLowerCase().trim();
            const filterType = String($('#filterType').val() || '').toLowerCase().trim();
            ...
            $('#creditsTableBody tr:not(#noResultRow)').each(function() {
                const $row = $(this);
                const dateValue = String($row.find('.date-cell').data('date') || '').trim();
                const clientValue = String($row.find('.client-cell').data('client') || '').toLowerCase();
                const typeValue = String($row.find('.type-cell').data('type') || '').toLowerCase();
                const montantValue = parseFloat($row.find('.montant-cell').data('montant')) || 0;
                const rembourseValue = parseFloat($row.find('.montant-cell').data('rembourse')) || 0;
                ...
                if (showRow && filterClient && !clientValue.includes(filterClient)) {
                    showRow = false;
                }

                if (showRow && filterType && !typeValue.includes(filterType)) {
                    showRow = false;
                }

                if (showRow && filterMontant !== null && !isNaN(filterMontant) && montantValue != filterMontant) {
                    showRow = false;
                }
                ...
                if (showRow) {
                    $row.show();
                    visibleCount++;
                    totalCredit += montantValue;
                    totalRemboursement += rembourseValue;
                } else {
                    $row.hide();
                }
            });

            $('#creditCount').text(visibleCount);
            $('#totalCredit').text(totalCredit.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ' '));
            $('#totalRemboursement').text(totalRemboursement.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ' '));
            $('#totalReste').text((totalCredit - totalRemboursement).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ' '));

            if (visibleCount === 0 && (filterDate || filterClient || filterType || filterMontant !== null)) {
                $('#msg').html('<i class="zmdi zmdi-info"></i> Aucun crédit ne correspond aux critères de recherche');
        ...
        // =================================================================
        $('#filterDate, #filterClient, #filterType, #filterMontant').on('input change', function() {
            saveFilters();
            filterCredits();
        });
        ...
        $("#print").click(function(e) {
            e.preventDefault();
            $.get("{{ url('/get_liste_credit') }}", {}, function(response) {
                $("#bloc_1").hide();
                $("#bloc_2").hide();
                $("#bloc_3").hide();
                $("#bloc_4").show();
        ...
        // Calcul automatique du solde (entrée + sortie)
        ...
        $("#save").click(function(e) {
            ...
            if (date_credit.trim().length == 0) {
            ...
            } else if (nom_credit.trim().length == 0) {
                ...
                $('#msg').css('display', 'flex');
                setTimeout(() => { $('#msg').html(''); $('#msg').css('display', 'none'); }, 9000);
            } else if (entree.trim().length == 0) {
                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Completez le montant');
                $('#msg').css('display', 'flex');
                setTimeout(() => { $('#msg').html(''); $('#msg').css('display', 'none'); }, 9000);
            } else if (entree.trim() <= 0) {
                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Le montant dois être supérieur à 0');
                $('#msg').css('display', 'flex');
                setTimeout(() => { $('#msg').html(''); $('#msg').css('display', 'none'); }, 9000);
            } else if (taux_credit.trim().length == 0) {
                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Completez taux du crédit');
                $('#msg').css('display', 'flex');
                setTimeout(() => { $('#msg').html(''); $('#msg').css('display', 'none'); }, 9000);
            } else {
                // Vérifications supplémentaires avec les champs masqués (si nécessaires)
                // On envoie directement car les validations principales sont faites
