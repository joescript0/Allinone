...
<section class="content">
                                ...
                                <?php if ((Writes::where(["ressource_id" => $ressource_id_1, "groupe_id" => $groupe_user_id])->get()->count() != 0) || (Auth::user()->role == 0)) { ?>
                                    ...
                                    <?php if (($add ==  1) || (Auth::user()->role == 0)) { ?>
                                        ...
                                        <a id="print" class="btn-primary btn-sm" href="">
                                            <i class="zmdi zmdi-cast"></i> Qr code
                                        </a>
                                    <?php } else { ?>
                                        <a id="add_r" href="">
                                            <i class="zmdi zmdi-accounts-add"></i> Ajouter
                                        ...
                                        &nbsp;
                                        <a id="print_r" href="">
                                            <i class="zmdi zmdi-code-setting"></i> Imprimer
                                        </a>
                                    <?php } ?>
                                <?php } ?>
                            </div>
    ...
    <div style="margin-top: 30px;" class="container">
        <div class="row">
            <div class="col-lg-12">
                <h6 style="color:rgba(0, 0, 0, 0.6);">{{ strtoupper(Auth::user()->name) }}&nbsp; <i class="zmdi zmdi-chevron-right"></i> &nbsp; Configurer poste</h6>
            </div>
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <select id="lieu_id" name="lieu_id" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <select id="client_id" name="client_id" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control">
                    ...
                    <div style="margin-top: -20px;" class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="nom" name="nom" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Nom (Ex : Mike alfa)">
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <textarea style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);"
                                    class="form-control" placeholder="Description (Ex : Quartier, Av, N° etc....)" name="description" id="description" cols="2" rows="1"></textarea>
                            </div>
                        </div>
                    </div>
                    <div style="margin-top: -20px;" class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="latitude" name="latitude" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Latitude (Ex : 48.8566)" value="">
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                                <label class="text-info" style="font-weight: bold;margin-top: 16px;"><i class="zmdi zmdi-pin"></i> Longitude </span></label>
                                <input type="text" id="longitude" name="longitude" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Longitude (Ex : 2.3522)" value="">
                            </div>
                        </div>
                    </div>
...
<span id="listesfactures_id" style="display: none;"></span>
    ...
    <div class="modal fade" id="cloturee" tabindex="-1">
        <div class="modal-dialog modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title pull-left text-center" style="font-weight: bold;font-size: 16px;">Ce poste est
                        deja cloturé </h5>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12" style="font-weight: bold;color:black;">
                            <label class="text-info" style="font-weight: bold;margin-top: 16px;"><i
                                            class="zmdi zmdi-info-circle"></i> Voulez-vous la liste de facture de quelle activité ? </span></label>
                                    <select style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" id="activite_id_a" name="activite_id_a" class="form-control"
...
<script>
    ...
    $("#save").click(function(e) {
        e.preventDefault();
        var lieu_id = $("#lieu_id").val();
        var client_id = $("#client_id").val();
        var nom = $("#nom").val();
        var data = $("#form_add").serialize();
        if (lieu_id.trim().length == 0) {
            $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Selectionnez le lieu');
            ...
            if (client_id.trim().length == 0) {
                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Selectionnez le client');
                $('#msg').css('color', "#ff6b68");
                setTimeout(() => {
                    $('#msg').html("");
                }, 9000);
                ...
                if (nom.trim().length == 0) {
                    $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Completez le nom du poste');
                    $('#msg').css('color', "#ff6b68");
                    setTimeout(() => {
                        $('#msg').html("");
                    }, 9000);
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/check_poste_existe",
                        data: data,
                        success: function(response) {
                            $("#save").attr("disabled", false);
                            if (response == 1)
                            {
                                $('#msg').html('<i class="zmdi zmdi-close-circle"></i> Ce poste est deja enregistré');
                                $('#msg').css('color', "#ff6b68");
                                setTimeout(() => {
                                    $('#msg').html("");
                                }, 9000);
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        ...
                                        $("#latitude").val("");
                                        $("#longitude").val("");
                                        $('#msg').html('<i class="zmdi zmdi-check-circle"></i> Poste ajouté avec succès');
    ...
    // ========== CODE POUR LA CARTE (géolocalisation auto sans marqueur, clic manuel, boutons overlay) ==========
    // Coordonnées par défaut du secteur de l'application (modifiez selon votre zone)
    var defaultLat = -4.4419;   // Kinshasa (exemple)
    var defaultLng = 15.2663;
    var defaultZoom = 13;

    var map = L.map('map').setView([defaultLat, defaultLng], defaultZoom);
    var currentTileLayer;
    var marker = null; // Pas de marqueur initial
    ...
    // Fonction pour remplir les champs et placer/mettre à jour le marqueur
    ...
    // Clic sur la carte : position manuelle
    ...
    // Géolocalisation automatique : centre la carte sur votre position (sans marqueur, sans remplir les champs)
    ...
    // Bouton Position actuelle (dans la barre d'outils) : remplit les champs, place marqueur, centre
    ...
    // Réinitialiser la vue (bouton overlay)
    ...
    // Synchronisation initiale : si les champs contiennent déjà des valeurs (cas d'édition), on place le marqueur
    (function() {
