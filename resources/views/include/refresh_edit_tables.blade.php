    ...
    <div style="display: none;" class="col-6">
        <div class="form-group">
            ...
            <input type="text" id="id" name="id"
                ...
                class="form-control" placeholder="Nom (Ex : Mgm congo)" value="<?= $tables->id ?>">
        </div>
    </div>
    <div class="row">
        <div class="col-6">
            <div class="form-group">
                ...
                <input type="text" id="edit_nom" name="edit_nom"
                    style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);"
                    class="form-control" placeholder="Libelle (Ex : MGM)" value="<?= $tables->nom ?>">
            </div>
        </div>
        <div class="col-6">
            <div class="form-group">
                ...
                <select id="edit_point_vente_id" name="edit_point_vente_id"
                    style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);"
                    class="form-control">
                    @foreach ($point_ventes as $data)
                        @if ($data->id == $tables->pointdeventes_id)
        ...
        <div class="col-6">
            <div class="form-group">
                ...
                <textarea style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);"
                    ...
                    rows="1"><?= $tables->description ?></textarea>
            </div>
        </div>
    </div>
    <div class="row">
...
<script>
    ...
    $("#edit_save").click(function(e) {
        ...
        var point_vente_id = $("#edit_point_vente_id").val();
        var description = $("#edit_description").val();
        var data = $("#form_edit").serialize();
        if (nom.trim().length == 0) {
            $('#edit_msg').html('<i class="zmdi zmdi-close-circle"></i> Completez le type de documents');
            ...
            {
                $('#edit_msg').html('<i class="zmdi zmdi-close-circle"></i> Sélectionnez un point de vente');
                $('#edit_msg').css('color', "#ff6b68");
                setTimeout(() => {
                    $('#edit_msg').html("");
                }, 9000);
                        ...
                        if (response == 1) {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    ...
                                    $('#edit_msg').html(
                                        '<i class="zmdi zmdi-check-circle"></i> Table modifiée avec succès'
                                    );
