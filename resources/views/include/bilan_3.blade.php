...
<div style="margin-bottom: 100px;">
    ...
    <div class="row">
        <div class="col-12">
            <h4 style="text-align: right;color: black;font-weight: bold;">TOTAL {{ $annees->annees }} : <?= number_format($total_general, 2, ',', ' ') ?>$</h4>
        </div>
    </div>
    <hr style="margin-top: -2px;border: 1px solid black">
    <?php foreach ($mois as  $m) {?>
        <?php if(Entres::where(["annee_id" => $annees->id, "moi_id" => $m->id])->get()->count() != 0){ ?>
            <?php
                $t = 0;
            ...
            <div class="row">
                <div class="col-12">
                    <h6 style="text-align: right;color: black;font-weight: bold;"><span>SOLDE INITIAL : {{ number_format(Soldes::where(["annee_id" => $annees->id, "moi_id" => $m->id])->first()["solde_initial"], 2, ',', ' ') }}$</span></h4>
                </div>
            </div>
            ...
            <div class="row" style="margin-top: 10px;">
                <div class="col-12">
                    <h6 style="text-align: right;color: black;font-weight: bold;"><span class="text-info">SOLDE FINAL : <?= number_format($t, 2, ',', ' ') ?>$</span>.</h4>
                </div>
            </div>
            <hr style="margin-top: -2px;border: 1px solid black">
        <?php } ?>
