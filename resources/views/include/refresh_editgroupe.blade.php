...
<form id="form_edit" action="#" method="post">
    ...
    <div style="display: none;" class="col-6">
            <div class="form-group">
                ...
                <input type="text" id="id" name="id" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Nom (Ex : Mgm congo)" value="<?= $groupes->id ?>">
            </div>
        </div>
    <div class="row">
        <div class="col-6">
            <div class="form-group">
                ...
                <input type="text" id="edit_nom" name="edit_nom" style="font-weight: bold;border-radius:5px;padding-left: 5px;border: 1px solid rgba(0, 0, 0, 0.2);" class="form-control" placeholder="Nom (Ex : Admin)" value="<?= $groupes->nom ?>">
            </div>
        </div>
    </div>
    <div class="row">
        ...
        if (nom.trim().length == 0) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response == 1) {
                        $('#edit_msg').html('<i class="zmdi zmdi-close-circle"></i> Ce rôle existe déjà');
                        $('#edit_msg').css('color', "#ff6b68");
                        setTimeout(() => {
                            $('#edit_msg').html("");
                        }, 9000);
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $('#edit_msg').html('<i class="zmdi zmdi-check-circle"></i> Rôle modifié avec succès');
